        ...
        @if ($students->hasPages())
            <div class="mt-4">
                <ul class="flex justify-center space-x-2">
                    <li>
                        <a href="{{ $students->url(1) }}" aria-label="First"
                            class="px-3 py-1 border rounded-md hover:bg-gray-200
                    {{ $students->onFirstPage() ? 'cursor-not-allowed text-gray-400' : '' }}">
                            << </a>
                    </li>

                    <li>
                        <a href="{{ $students->previousPageUrl() }}" aria-label="Previous"
                            class="px-3 py-1 border rounded-md hover:bg-gray-200
                    {{ $students->onFirstPage() ? 'cursor-not-allowed text-gray-400' : '' }}">
                            Previous
                        </a>
                    ...
                    @for ($page = 1; $page <= $students->lastPage(); $page++)
                        <li>
                            <a href="{{ $students->url($page) }}"
                                class="px-3 py-1 border rounded-md hover:bg-gray-200
                        {{ $students->currentPage() == $page ? 'bg-gray-300 text-gray-700' : '' }}">
                                {{ $page }}
                            </a>
                        </li>
                    @endfor
                    ...
                    <li>
                        <a href="{{ $students->nextPageUrl() }}" aria-label="Next"
                            class="px-3 py-1 border rounded-md hover:bg-gray-200
                    {{ $students->hasMorePages() ? '' : 'cursor-not-allowed text-gray-400' }}">
                            Next
                        </a>
                    </li>

                    <li>
                        <a href="{{ $students->url($students->lastPage()) }}" aria-label="Last"
                            class="px-3 py-1 border rounded-md hover:bg-gray-200
                    {{ $students->hasMorePages() ? '' : 'cursor-not-allowed text-gray-400' }}">
                            >>
                        </a>
